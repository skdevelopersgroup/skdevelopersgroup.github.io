...
if($_SESSION['uid']=="")
 ...
 $useridm=$_REQUEST['uid'];
	$messageviewedstatus=$_REQUEST['messageviewedstatus'];
	if($messageviewedstatus=="viewed")
	{
	 $updmessagestatus = "update userdetails set messagestatus ='viewed' where`uid`='$useridm'"; 	 
				$updatemessagestatus = mysql_query($updmessagestatus) or die('Update Query error:');
	}
...
$ins = "INSERT INTO messages(`fromuid`,`touid`,`messages`,`image`,`dated`) 
	 VALUES('$useridmd','$useridm','$messages','','$userdatetime')";
...
//	 echo $sqlins;
	 ...
	 elseif(($messages=="") && ($imgfile!=""))
	 {
	  $imgfile="".$imgfile;	
			...
			move_uploaded_file($_FILES['imgfile']['tmp_name'], $target_path);
$ins = "INSERT INTO messages(`fromuid`,`touid`,`messages`,`image`,`dated`) 
	 VALUES('$useridmd','$useridm','','$imgfile','$userdatetime')";
	 $sqlins= mysql_query($ins) or die('Insert Query error:');	
	 $msg = "<font color=#FF0000>Successfully Inserted..</font>";
	 }
	 else
	 {
	  $imgfile="".$imgfile;	
			...
			move_uploaded_file($_FILES['imgfile']['tmp_name'], $target_path);
$ins = "INSERT INTO messages(`fromuid`,`touid`,`messages`,`image`,`dated`) 
	 VALUES('$useridmd','$useridm','$messages','$imgfile','$userdatetime')";
	  $sqlins= mysql_query($ins) or die('Insert Query error:');	
	 $msg = "<font color=#FF0000>Successfully Inserted..</font>";
	 }
	  $show2= "select * from messages where ((fromuid=1 or fromuid=2 or fromuid=3 or fromuid=4 or fromuid=5) and touid='$useridm') or (fromuid='$useridm' and touid=1 or touid=2 or touid=3 or touid=4 or touid=5) order by dated desc limit 1";
...
$nums2 = mysql_num_rows($res2);
 ...
 $lastmid=$row2['mid'];
 		...
 		?>
		<script language="javascript">window.location='replyorviewmessages.php?uid=<? echo $useridm; ?><? if($lastmid!="") { ?>#<? echo $lastmid; ?><? } ?>';</script>
	<?	
}
...
{
    var newWindow = window.open("New window", "New window", 
        ...
        <? }?>
          <div class="add_message_bx">            <form action="" method="post" enctype="multipart/form-data" name="form1" id="form1" onSubmit="return val();">
              <table width="880" border="0" cellspacing="0" cellpadding="0">
                ...
                <tr>
                  <td valign="top"><label>
                   <a name="reply" id="reply"></a>  <textarea name="message" cols="45" rows="5" class="user_management_fieldbx" id="message"></textarea>
                  </label></td>
                  </tr>
                <tr>
